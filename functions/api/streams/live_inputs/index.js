 ...
 */
 async function gatherResponse(response) {
  ...
  /**
   * Handle a request to the streams endpoint by sending a get streams request to the CloudFlare API.
  ...
  async function handleRequest(request) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${headers.get("account-id")}/stream/live_inputs`,
        init
      );
      ...
      headers2.set("Access-Control-Allow-Origin", "http://localhost:3000");
  ...
  /**
   * Handles a request to the streams endpoint.
   * @param {*} context
  ...
  if (request.method === "OPTIONS") {
    return new Response("", {
      ...
      headers: {
        ...
        "Access-Control-Allow-Headers": "Content-Type, Authorization, account-id",
      },
    });
  }
