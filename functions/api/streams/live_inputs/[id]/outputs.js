 ...
 */
 async function gatherResponse(response) {
  ...
  async function handleRequest(request, params) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${headers.get("account-id")}/stream/live_inputs/${params.id}/outputs`,
        init
      );
  ...
  /**
   * Handle a request to the outputs endpoint by sending a post outputs request to the CloudFlare API.
   * @param {*} request Contains the request object from the incoming request.
   * @returns The response to the request.
   */
   async function handleCreate(request, params) {
    const { headers } = request;
    try {
      // get accounts
      const init = {
